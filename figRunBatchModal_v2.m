function varargout = figRunBatchModal_v2(varargin)
% FIGRUNBATCHMODAL_V2 MATLAB code for figRunBatchModal_v2.fig
%      FIGRUNBATCHMODAL_V2, by itself, creates a new FIGRUNBATCHMODAL_V2 or raises the existing
%      singleton*.
%
%      H = FIGRUNBATCHMODAL_V2 returns the handle to a new FIGRUNBATCHMODAL_V2 or the handle to
%      the existing singleton*.
%
%      FIGRUNBATCHMODAL_V2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FIGRUNBATCHMODAL_V2.M with the given input arguments.
%
%      FIGRUNBATCHMODAL_V2('Property','Value',...) creates a new FIGRUNBATCHMODAL_V2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before figRunBatchModal_v2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to figRunBatchModal_v2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help figRunBatchModal_v2

% Last Modified by GUIDE v2.5 10-Jul-2026 21:28:15

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @figRunBatchModal_v2_OpeningFcn, ...
                   'gui_OutputFcn',  @figRunBatchModal_v2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before figRunBatchModal_v2 is made visible.
function figRunBatchModal_v2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to figRunBatchModal_v2 (see VARARGIN)

% Choose default command line output for figRunBatchModal_v2
...
% UIWAIT makes figRunBatchModal_v2 wait for user response (see UIRESUME)
...
data.outputdir = data.editOutputPath.String;
data.filenames = {};
...
function varargout = figRunBatchModal_v2_OutputFcn(hObject, eventdata, handles) 
...
data_parent.batchoutputdir = data.outputdir;

% get the checkboxes ticked (1-ncheck)
ncheck = 16;
checked = table( ...
    zeros(ncheck,1), ...          % order (int)
    strings(ncheck,1), ...        % tag (string)
    zeros(ncheck,1), ...
    'VariableNames', {'order','tag', 'value'} ...
);

for cb=1:ncheck % warning: this is hard coded!
    checked.order(cb) = cb;
    checked.tag(cb) = sprintf('popupmenu%d', cb);
    checked.value(cb) = get(eval(sprintf('data.popupmenu%d',cb)),'Value');
end
data_parent.batchchecked = checked;
    
% store the data in the parent object
guidata(hObject, data);
...
if ischar(filenames)
    filenames = {filenames};
elseif iscell(filenames) && ~ischar(filenames{1})
...
if ~isfield(data, 'closerq') || data.closerq<2
    ...
    try
        ...
        % general error
        if ~isfield(data, 'closerq')
            data.closerq = 1;
        else
            data.closerq = data.closerq+1;
        end
        guidata(hObject, data)
    end
else
    % close anyway after a few tries
    delete(hObject);
end
...
data = guidata(hObject);
% fill the pathname with the new string
data.pathname = data.editSelectFilePath.String;
guidata(hObject, data);
...
% --- Executes on button press in pushbuttonSelectDir.
function pushbuttonSelectDir_Callback(hObject, eventdata, handles)
% hObject    handle to pushbuttonSelectDir (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

data = guidata(hObject);

if ~isfield(data, 'outputpath') || isempty(data.outputpath)
    startdir = data.INIDIR;
else
    startdir = data.outputpath;
end
outputdir = uigetdir(startdir, 'Pick an output folder');
if ~isempty(outputdir)
    data.outputdir = outputdir;
end
data.editOutputPath.String = outputdir;
guidata(hObject, data);
